...
namespace DaleranGames.TBSFramework
{
    ...
    public class BuildFeature : FeatureType, ICancelable, IWorkable, IPlaceable
    {
        ...
        {
            if (tile.Owner.Goods.CanProcessNow(initialBuildCosts.GetAllCostsAsTransaction(tile.Owner.Stats)) && tile.Owner.Goods.CanProcessNow(perTurnBuildCosts.GetAllCostsAsTransaction(tile.Owner.Stats)) && validLands.Contains(tile.Land))
            {
                return true;
            }
            else
        ...
        {
            return perTurnBuildCosts.GetAllCostsAsTransaction(tile.Owner.Stats)[0].Value;
        }
        ...
        public override TileGraphic GetWorkIcon(HexTile tile)
        {
            if (tile.Work.Paused)
                ...
                return GameDatabase.Instance.TileGraphics["Icon_16px_Good_Labor"];
        }
        #endregion
    }
}
